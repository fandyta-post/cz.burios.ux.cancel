package cz.burios.ux.cancel.model.sync.google.calendar;
...
/**
 * A sample that demonstrates how to efficiently sync <a href=
 * "https://developers.google.com/google-apps/calendar/v3/reference/">Calendar
 * resource</a> using sync tokens.
 *
...
public class SyncTokenSample {
	...
	/**
	 * Syncs events from the user's primary calendar to a local datastore. A full
	 * sync is performed on the first run, with incremental syncs on subsequent
	 * runs.
	 */
	private static void run() throws IOException {
		...
		if (syncToken == null) {
			...
			// Set the filters you want to use during the full sync. 
			// Sync tokens aren't compatible with most filters, 
			// but you may want to limit your full sync to only a certain date range.
		...
		// Store the sync token from the last request to be used during the next
		// execution.
	...
	/**
	 * Sync an individual event. In this example we simply store it's string
	 * represenation to a file system data store.
	 */
	private static void syncEvent(Event event) throws IOException {
		if ("cancelled".equals(event.getStatus()) && eventDataStore.containsKey(event.getId())) {
			...
			System.out.println(String.format("Syncing event: ID=%s, Name=%s", event.getId(), event.getSummary()));
		}
	}
}
